...
export default function ChapterReaderWithProgress({
  mangaId,
  chapterId,
}: {
  mangaId: string
  ...
  const progressThreshold = 0.7 // Mark as read when 70% of pages have been viewed
  ...
  useEffect(() => {
    ...
    loadChapter()
  }, [chapterId, retryCount, isForcingRefresh, mangaId])
  ...
  // Set up intersection observer for lazy loading and reading progress tracking
  useEffect(() => {
  ...
  // Mark chapter as read when enough pages have been viewed
  useEffect(() => {
    if (!isSignedIn || hasMarkedAsRead || loading || pages.length === 0) return

    const uniqueVisiblePages = [...new Set(visiblePages)]
    const percentageViewed = uniqueVisiblePages.length / pages.length

    // Mark as read when user has viewed enough pages
    if (percentageViewed >= progressThreshold) {
  ...
  }, [visiblePages, pages.length, isSignedIn, hasMarkedAsRead, loading, mangaId, chapterId])
        ...
        <div className="flex items-center gap-2">
          {isSignedIn && (
            <Button
              ...
              className={`flex items-center gap-2 ${hasMarkedAsRead ? "bg-green-500 hover:bg-green-600" : ""}`}
            >
              <Check className={`h-4 w-4 ${hasMarkedAsRead ? "text-white" : ""}`} />
